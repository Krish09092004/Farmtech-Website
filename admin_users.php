...
<section class="user-accounts">
   ...
   <div class="box-container" data-aos="zoom-in" class="div" data-aos-duration="1500">
      ...
      ?>
      <div class="box" style="<?php if($row['bid'] == $bid){ echo 'display:none'; }; ?>">
         <img src="images/<?= $row['picExt']; ?>" alt="">
         <p> user id : <span><?= $row['bid']; ?></span></p>
         <p> username : <span><?= $row['bname']; ?></span></p>
         <p> email : <span><?= $row['bemail']; ?></span></p>
         <P> user type: <span> Buyer </span></p>
         <a href="admin_users.php?delete=<?php echo $row['bid']; ?>" class="delete-btn" onclick="return confirm('delete this user?');">delete</a>
          </div>
      <?php
      }
      ?>
      ...
      $select_users= mysqli_query($conn, "SELECT * FROM `farmer`")or die('query failed');
      	while($row= mysqli_fetch_assoc($select_users)){
      ?>
      <div class="box" style="<?php if($row['fid'] == $fid){ echo 'display:none'; }; ?>">
         <img src="images/<?= $row['picExt']; ?>" alt="">
         <p> user id : <span><?= $row['fid']; ?></span></p>
         <p> username : <span><?= $row['fname']; ?></span></p>
         <p> email : <span><?= $row['femail']; ?></span></p>
         <p> User type: <span> Farmer </span></p>
         <a href="admin_users.php?delete1=<?php echo $row['fid']; ?>" class="delete-btn" onclick="return confirm('delete this user?');">delete</a>
        </div>
      <?php
      }
      ?>
