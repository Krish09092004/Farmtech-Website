...
if(isset($_POST['update_product'])){
   ...
   $update_product= "UPDATE fproduct SET product ='$name', pcat='$category', pinfo='$details', price='$price' where pid='$pid';";
...
<section class="update-product">
   ...
   ?>
   <form action="" method="post" enctype="multipart/form-data" data-aos="fade-up" class="div" data-aos-duration="1500">
      <input type="hidden" name="old_image" value="<?php echo $row['pimage']; ?>">
      <input type="hidden" name="pid" value="<?php echo $row['pid']; ?>">
      <img src="images/<?php echo $row['pimage']; ?>" alt="">
      <input type="text" name="name" placeholder="enter product name" required class="box" value="<?php echo $row['product']; ?>">
      <input type="number" name="price" min="0" placeholder="enter product price" required class="box" value="<?php echo $row['price']; ?>">
      <select name="category" class="box" required>
         <option selected><?php echo $row['pcat']; ?></option>
         <option value="vegitables">vegitables</option>
         <option value="fruits">fruits</option>
         <option value="grains">grains</option>
      </select>
      <textarea name="details" required placeholder="enter product details" class="box" cols="30" rows="10"><?php echo $row['pinfo']; ?></textarea>
